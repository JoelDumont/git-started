...
A **tag** is a reference to a specific point in a Git repository's history. It's commonly used to mark important milestones such as releases or versions of a project. The key difference from a branch is that they are immutable and don't move forward when new changes are added.
...
|===
|xref:02_Git_workflow.adoc[<- Back to Git Workflow]
|xref:04_Vibe_coding.adoc[Continue to Vibe-Coding ->]
|===
